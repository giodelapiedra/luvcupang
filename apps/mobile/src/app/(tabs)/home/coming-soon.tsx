...
export default function ComingSoonScreen() {
  ...
  return (
    <ScreenWrapper background="#FFFFFF" edges={['left', 'right']}>
      <PageHeader title={service.label} />

      <View className="flex-1 items-center px-6 pt-10">
        {/* Animated rings + flat icon tile */}
        <View className="h-44 w-44 items-center justify-center">
          ...
          >
            <Ionicons name={service.iconName} size={36} color="#FFFFFF" />
          </View>
        </View>

        <Text
          className="mt-6 text-[11px] font-extrabold uppercase tracking-widest"
        ...
        <Text className="mt-2 text-center text-sm leading-5 text-slate-600">
          We&apos;re building <Text className="font-bold">{service.label}</Text> — a digital
          barangay service designed to make your life easier.
        </Text>

        {/* Feature list — flat white card, subtle border */}
        <View className="mt-7 w-full rounded-2xl border border-slate-100 bg-white p-4">
          {[
            ...
            <View key={line} className="mb-3 flex-row items-start gap-3 last:mb-0">
